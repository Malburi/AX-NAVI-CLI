/*
 * 인코딩 보존 훅의 명령형 입구 — `node legacy-encoding-hook.mjs pre|post [plugin]`.
 *
 ...
 * 둘이 함께 돌면 같은 원본을 두 번 쓴다.
 */
import { encodingPostToolUse, encodingPreToolUse } from "./legacy-encoding.mjs";
...
process.stdin.on("end", () => {
  ...
  let out = {};
  try {
    out = phase === "post" ? encodingPostToolUse(event) : encodingPreToolUse(event);
  } catch {
