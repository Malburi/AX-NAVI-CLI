...
/*
 * 위임 경로는 설치본을 --plugin-dir 로 물려 서브에이전트 이름을 얻는다.
 * 매니페스트가 빠지면 claude 가 플러그인으로 인식하지 못하고, 그러면
 * Agent(subagent_type="ax-navi:analyzer") 가 전부 not found 로 끝난다 —
 * 팬아웃이 조용히 사라지고 오케스트레이터가 혼자 다 한다(실측).
 */
test("배포본이 그 자체로 적재 가능한 플러그인이다", () => {
  assert.ok(manifest.files.includes(".claude-plugin/"), "플러그인 매니페스트가 배포본에서 빠졌다");
});
