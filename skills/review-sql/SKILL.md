...
네임스페이스를 지정한 호출은 에이전트 지침이 자동으로 로드되므로 프롬프트에 절차를 인라인하지 않고 인자만 전달한다. 플러그인 네임스페이스 지정을 지원하지 않는 호스트에서는 `general-purpose`로 폴백하되 프롬프트에 "`agents/sql-reviewer.md`의 지침을 읽고 그대로 따른다"를 명시한다.
...
주석 없는 대형 프로시저·트리거·동적 SQL 등 의도 파악이 어려운 레거시 SQL이면:
- "legacy-decoder로 상세 해석 권고" 안내 (리뷰는 위험 평가에 집중하고, 로직 역공학은 legacy-decoder로 위임)
...
sql-reviewer 리포트의 확정 발견·조건부 권고·수정안 SQL 3분법을 그대로 보고에 옮긴다. 사용자가 "발견만"을 요청하면 조건부 권고·수정안을 생략하고 확정 발견만 보고한다.

### 인덱스/스키마 캐시 의존

`_workspace/index/schema.json` 이 있어야 정확한 분석. 없으면 → "DB 접속하여 스키마 추출 필요" 안내.

`_workspace/index/sql_usage.json` 도 사용처(호출 지점) 역추적의 근거다. 없거나 stale(HEAD와 불일치)이면 사용처를 grep으로 보완하되, 리포트의 `사용처` 차원에 반드시 **"인덱스 없음 — 사용처 미확정"**을 표시한다. "사용처 0건"을 "아무도 안 쓰는 안전한 SQL"로 오인하지 않게, 인덱스 부재와 실제 미사용을 구분해 보고한다.
