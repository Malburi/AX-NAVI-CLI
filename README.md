...
| 모드 | 하는 일 | |
|---|---|---|
| **기본** | 에이전트가 선언한 도구 그대로 | 강제 |
| **읽기 전용** | 무엇도 고치지 않는다 — 쓰기 도구가 실제로 목록에서 빠진다 | 강제 |
| **빠름** | 영향도·안전 게이트를 건너뛴다 | 지침 |

```
AX-NAVI (읽기 전용) > 이 변경 안전해?
```

**"강제"와 "지침"을 갈라 적습니다.** 읽기 전용은 런타임이 도구를 막습니다.
빠름은 모델에게 주는 절차 지침이라 지키지 않을 수 있습니다 —
빠름에서도 스키마·API 계약·트랜잭션 경계 변경과 3개 이상 파일 수정은 멈추게 합니다.
...
턴이 도는 동안 상태가 한 줄로 갱신됩니다.

```
⠹ harness-init › 백엔드 분석 › analyzer · 8m 7s · ↓ 30.6k tokens · Read · ⌨ /index refr
```

작업 중에 친 글은 **사라지지 않고 줄을 섭니다.** 턴이 끝나면 순서대로 실행됩니다.
`⌨` 뒤가 지금 치고 있는 글입니다.
...
```
? 이 프로젝트를 어떤 구성으로 초기화할까요?
  단일 프로젝트로 초기화 (Recommended)
❯ 서버·클라이언트 함께 초기화 (모노레포)
  서버·클라이언트 각각 초기화 후 연결 (1:1)
  기타 (부분 범위 / 허브형 1:N)
  ↑↓ 이동 · Enter 선택 · Esc 건너뜀
```
...
- **접힌 출력 펼치기.** `… +20줄` 을 펼치는 조작은 없습니다. 전체는 `--verbose` 로 봅니다.
