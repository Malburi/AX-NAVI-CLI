...
- **HOLD(`UNVERIFIED`)가 나왔다** — 실행 가능한 테스트·빌드 명령이 없거나(`detected count: 0`) 실행하지 못한 것이다. "테스트가 없어 실행하지 않음"은 PASS가 아니다. 수동 검증 시나리오를 확보하거나 `test-generator`로 회귀 테스트를 만든 뒤 "이 변경 다시 평가해줘"로 재평가한다.
