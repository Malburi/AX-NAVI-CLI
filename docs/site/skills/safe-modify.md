...
| **HOLD** | 검증을 실행하지 못했거나 어댑터가 PARTIAL/UNSUPPORTED면 위험 점수가 낮아도 HOLD(`UNVERIFIED`). `구조화 패턴 미검증` 폴백도 자동 GO 대상이 아니다. |
...
- **미실행은 PASS가 아니다.** `detected`가 비어 있으면(`count: 0`) 수동 검증 시나리오를 확보하기 전 PASS로 간주하지 않는다. 실행할 수 없거나 assertion까지 도달하지 못한 검사도 PASS가 아니다.
