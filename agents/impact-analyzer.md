...
| 규모 | 범위 |
|---|---|
| `small` | 인덱스 질의(`callers`·`sql`·`column`·`table`)로 직접 영향과 같은 SQL·화면을 쓰는 곳을 먼저 잡는다. 그 결과로 좁혀진 파일의 필요한 줄만 연다. Step 3 간접 추적은 1홉까지. Step 8.5 파트너 영향은 API 계약(엔드포인트 경로·요청/응답 필드)이 바뀔 때만 보고, 아니면 "API 계약 변경 없음 — 파트너 영향 없음"으로 적는다. 도구 호출은 약 15회 안에서 끝낸다. |
