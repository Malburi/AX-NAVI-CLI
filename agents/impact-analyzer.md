...
입력에 `맥락: _workspace/reports/context_<slug>.md`가 있으면 가장 먼저 읽는다. 오케스트레이터가 이미 읽은 파일(`원문 확인`)과 확인한 사실(`핵심 사실`)은 다시 탐색하지 않고 그대로 근거로 쓴다. 새로 확인할 것만 찾는다.

입력의 `규모`에 따라 범위를 맞춘다. 실측: 컬럼 1개를 SELECT 끝에 더하는 변경에 짝 저장소까지 뒤지며 도구를 38번, 10분 가까이 썼다.

| 규모 | 범위 |
|---|---|
| `small` (safe-modify는 small이면 이 에이전트를 부르지 않는다. 오케스트레이터가 직접 확인하다가 예상과 달라 올렸거나, 다른 스킬이 small로 부를 때) | 인덱스 질의(`callers`·`sql`·`column`·`table`)로 직접 영향과 같은 SQL·화면을 쓰는 곳을 먼저 잡는다. 그 결과로 좁혀진 파일의 필요한 줄만 연다. Step 3 간접 추적은 1홉까지. Step 8.5 파트너 영향은 API 계약(엔드포인트 경로·요청/응답 필드)이 바뀔 때만 보고, 아니면 "API 계약 변경 없음 — 파트너 영향 없음"으로 적는다. 도구 호출은 약 15회 안에서 끝낸다. |
| `normal` (또는 없음) | 아래 단계 전부 |

어느 쪽이든 확정 못 한 것은 `분석 한계`에 적고 넘어간다. 확정하려고 저장소 전체를 반복해서 grep 하지 않는다.
