...
- pattern-conformance가 `HOLD`이거나 있는 테스트/빌드/린트를 실행하지 않았으면(`UNVERIFIED`) 최소 **HOLD**
- pattern-conformance가 `FAIL`이거나 필수 검증 명령이 실패하면 **STOP**
- “테스트가 없어 실행하지 않음”은 PASS가 아니다. 프로젝트에 실행 가능한 검증 명령이 정말 없다면 근거를 기록하고 `검증 수단 없음`으로 표시한다. 이때 DB 스키마·트랜잭션·인증·공통 모듈 변경이면 **HOLD**, 그 밖은 점수로 판정한다.
